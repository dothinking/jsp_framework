...
class PriorityDispatchSolver(JSSolver):
    ...
    def __init__(self, name:str=None, problem:JSProblem=None, rule:str=None, fun_rule=None) -> None:
...
class DisPatchingRules:
    ...
    def PS(op:OperationStep, solution:JSSolution):
        '''Process Sequence.'''
        job = op.source.job
        return len(list(filter(lambda op: op.source.job==job, solution.ops)))

    @staticmethod
    def TPT(op:OperationStep, solution:JSSolution):
        '''Total Processing Time.'''
        job = op.source.job
        return sum(op.source.duration for op in solution.ops if op.source.job==job)
